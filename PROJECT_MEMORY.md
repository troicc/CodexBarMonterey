...
- 最新本地验证安装包（0.10.0 / 202609251739）已上传草稿 `local-validation-202609251739`（release ID 396500306；编辑草稿可能改变 untagged 链接）：https://github.com/troicc/CodexBarMonterey/releases/tag/untagged-c32decb18d5ed3c7cff8 。附件为约 22 MiB 的 ZIP、BUILD-METADATA.json、SHA256SUMS，共 3 个；远端 state/size/SHA-256 全部与本地一致，压缩包已实际解压并对比完整 bundle 文件哈希、模式、符号链接和签名。元数据的 32 个 Swift source hashes 逐项匹配功能提交 dea5ea5，主程序/helper hash 与来源说明均保存。
